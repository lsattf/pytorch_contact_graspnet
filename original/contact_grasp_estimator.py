...
import config_utils
from data import farthest_points, distance_by_translation_point, preprocess_pc_for_inference, regularize_pc_point_count, \
    depth2pc, reject_median_outliers
...
class GraspEstimator:
    ...
    def __init__(self, cfg):
        ...
        if 'surface_grasp_logdir_folder' in cfg:
            # for sim evaluation
            self._contact_grasp_cfg = config_utils.load_config(cfg['surface_grasp_logdir_folder'], batch_size=1,
        ...
        self._num_input_points = self._contact_grasp_cfg['DATA']['raw_num_points'] if 'raw_num_points' in \
                                                                                      self._contact_grasp_cfg[
                                                                                          'DATA'] else \
    ...
    def build_network(self):
        ...
        print("--- Get model")
        # Get model
        end_points = self._model_func.get_model(self.placeholders['pointclouds_pl'],
                                                self.placeholders['is_training_pl'], global_config, bn_decay=bn_decay)

        tf_bin_vals = self._model_func.get_bin_vals(global_config)
        offset_bin_pred_vals = tf.gather_nd(tf_bin_vals,
                                            tf.expand_dims(tf.argmax(end_points['grasp_offset_head'], axis=2),
                                                           axis=2)) if global_config['MODEL']['bin_offsets'] else \
        end_points['grasp_offset_pred'][:, :, 0]

        grasp_preds = self._model_func.build_6d_grasp(end_points['approach_dir_head'], end_points['grasp_dir_head'],
                                                      end_points['pred_points'], offset_bin_pred_vals,
        ...
        self.model_ops = {'pointclouds_pl': self.placeholders['pointclouds_pl'],
                          ...
                          'offset_pred_idcs_pc': tf.argmax(end_points['grasp_offset_head'], axis=2) if
                          global_config['MODEL']['bin_offsets'] else None,
                          'offset_bin_pred_vals': offset_bin_pred_vals,
                          'grasp_preds': grasp_preds,
                          'step': step,
                          'end_points': end_points}

        self.inference_ops = [self.model_ops['grasp_preds'], self.model_ops['binary_seg_pred'],
        ...
        self.end_points = end_points # added myself
        self.offset_bin_pred_vals = offset_bin_pred_vals # added myself
    ...
    def predict_grasps(self, sess, pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        ...
        end_point_test = sess.run(self.end_points, feed_dict=feed_dict)
        offset_bin_pred_vals_test = sess.run(self.offset_bin_pred_vals, feed_dict=feed_dict)
        print('test end points---------------------------------------')
        print(end_point_test['grasp_dir_head'].shape)
        print(end_point_test['binary_seg_head'].shape)
        print(end_point_test['binary_seg_pred'].shape)
        print(end_point_test['grasp_offset_head'].shape)
        print(end_point_test['grasp_offset_pred'].shape)
        # print(end_point_test['grasp_offset_pred'][0][1])
        # print(end_point_test['grasp_offset_pred'][0][2])
        # print(end_point_test['grasp_offset_pred'][0][345])
        # print(offset_bin_pred_vals_test)
        # print(offset_bin_pred_vals_test.shape)
        print(end_point_test['approach_dir_head'].shape)
        print(len(end_point_test['pred_points'].shape))
        print('test end points---------------------------------------')
        ...
        if constant_offset:
            offset_pred = np.array([[self._contact_grasp_cfg['DATA']['gripper_width'] - self._contact_grasp_cfg['TEST'][
                'extra_opening']] * self._contact_grasp_cfg['DATA']['num_point']])
        ...
        with_replacement = self._contact_grasp_cfg['TEST']['with_replacement'] if 'with_replacement' in \
                                                                                  self._contact_grasp_cfg[
                                                                                      'TEST'] else False
        ...
        selection_idcs = self.select_grasps(pred_points[:, :3], pred_scores,
                                            ...
                                            self._contact_grasp_cfg['TEST']['second_thres'] if 'second_thres' in
                                                                                               self._contact_grasp_cfg[
                                                                                                   'TEST'] else
                                            self._contact_grasp_cfg['TEST']['first_thres'],
        ...
        return pred_grasps_cam[selection_idcs], pred_scores[selection_idcs], pred_points[selection_idcs].squeeze(), \
               gripper_openings[selection_idcs].squeeze()

    def predict_scene_grasps(self, sess, pc_full, pc_segments={}, local_regions=False, filter_grasps=False,
                             forward_passes=1):
        """
        ...
        if local_regions:
            ...
            for k, pc_region in pc_regions.items():
                pred_grasps_cam[k], scores[k], contact_pts[k], gripper_openings[k] = self.predict_grasps(sess,
                                                                                                         pc_region,
                ...
                if k == 1:
                    ...
                    print(pred_grasps_cam[k].shape)
                    print('scores')
                    print(scores[k].shape)
                    print(scores[k])
                    print('contact_pts')
                    print(contact_pts[k].shape)
                    print('gripper_openings')
                    print(gripper_openings[k].shape)
                    print('------------------test')
        else:
            pc_full = regularize_pc_point_count(pc_full, self._contact_grasp_cfg['DATA']['raw_num_points'])
            pred_grasps_cam[-1], scores[-1], contact_pts[-1], gripper_openings[-1] = self.predict_grasps(sess, pc_full,
                                                                                                         convert_cam_coords=True,
                                                                                                         forward_passes=forward_passes)
        ...
        if filter_grasps:
            ...
            for k in segment_keys:
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    segment_idcs = self.filter_segment(contact_pts[j], pc_segments[k],
                    ...
                    print('skipping obj {} since  np.any(pc_segments[k]) {} and np.any(contact_pts[j]) is {}'.format(k,
                                                                                                                     np.any(
                                                                                                                         pc_segments[
                                                                                                                             k]),
                                                                                                                     np.any(
    ...
    def select_grasps(self, contact_pts, contact_conf, max_farthest_points=150, num_grasps=200, first_thres=0.25,
    ...
    def extract_point_clouds(self, depth, K, segmap=None, rgb=None, z_range=[0.2, 1.8], segmap_id=0,
    ...
    def predict_scene_grasps_from_depth_K_and_2d_seg(self, sess, depth, segmap, K, z_range=[0.2, 1.8],
                                                     local_regions=False, filter_grasps=False, segmap_id=0,
                                                     skip_border_objects=False, margin_px=5, rgb=None,
                                                     forward_passes=1):
        """ Combines converting to point cloud(s) and predicting scene grasps into one function """

        pc_full, pc_segments = self.extract_point_clouds(depth, K, segmap=segmap, segmap_id=segmap_id,
                                                         skip_border_objects=skip_border_objects, margin_px=margin_px,
                                                         z_range=z_range, rgb=rgb)

        return self.predict_scene_grasps(sess, pc_full, pc_segments, local_regions=local_regions,
