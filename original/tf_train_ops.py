...
def load_labels_and_losses(grasp_estimator, contact_infos, global_config, train=True):
    ...
    # Get labels
    # put all points in a scene(document) into the function
...
# key function to transform original label data
def load_contact_grasps(contact_list, data_config):
    ...
    for i,c in enumerate(contact_list):
        contact_directions_01 = c['scene_contact_points'][:,0,:] - c['scene_contact_points'][:,1,:] # two pairs of points
        all_contact_points = c['scene_contact_points'].reshape(-1,3)
        all_finger_diffs = np.maximum(np.linalg.norm(contact_directions_01,axis=1), np.finfo(np.float32).eps) # two points distance, shape = [point pairs]
        all_contact_directions = np.empty((contact_directions_01.shape[0]*2, contact_directions_01.shape[1],))
        all_contact_directions[0::2] = -contact_directions_01 / all_finger_diffs[:,np.newaxis] # unit vextor of the contact direction
        all_contact_directions[1::2] = contact_directions_01 / all_finger_diffs[:,np.newaxis] # unit vextor of the reverse contact direction
        ...
        all_pos_finger_diffs = all_finger_diffs[pos_idcs//2] # // floor division 15//2 = floor(7.5)= 7, two paired points with the same diffs
        all_pos_contact_dirs = all_contact_directions[pos_idcs]# two pair points with reverse contact directions
        all_pos_approach_dirs = all_approach_directions[pos_idcs//2] # two paired points with the same approach directions
        ...
        pos_contact_points.append(all_pos_contact_points[pos_sampled_contact_idcs,:]) # list that every element shape = (number_of_sampled_points, 3) (points may contain repeat samples)
        pos_contact_dirs.append(all_pos_contact_dirs[pos_sampled_contact_idcs,:]) # list that every element shape = (number_of_sampled_points, 3( (points may contain repeat samples)
        pos_finger_diffs.append(all_pos_finger_diffs[pos_sampled_contact_idcs]) # list that every element shape = (number_of_sampled_points, ) (points may contain repeat samples)
        pos_approach_dirs.append(all_pos_approach_dirs[pos_sampled_contact_idcs]) # list that every element shape = (number_of_sampled_points, 3) (points may contain repeat samples)
    ...
    with tf.device(device):
        tf_scene_idcs = tf.constant(np.arange(0,len(pos_contact_points)), tf.int32) # shape = (number_of_files, )
        tf_pos_contact_points = tf.constant(np.array(pos_contact_points), tf.float32) # shape = (number_of_files, number_of_sampled_points, 3)
        tf_pos_contact_dirs =  tf.math.l2_normalize(tf.constant(np.array(pos_contact_dirs), tf.float32),axis=2) # shape = (number_of_files, number_of_sampled_points, 3) useless normalization
        tf_pos_finger_diffs = tf.constant(np.array(pos_finger_diffs), tf.float32) # shape = (number_of_files, number_of_sampled_points)
        tf_pos_contact_approaches =  tf.math.l2_normalize(tf.constant(np.array(pos_approach_dirs), tf.float32),axis=2) # shape = (number_of_files, number_of_sampled_points, 3) useless normalization
